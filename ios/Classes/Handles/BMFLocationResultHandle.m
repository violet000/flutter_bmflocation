...
//        NSLog(@"bdmap_loc_flutter_plugin:updateLocationFaild:{%ld - %@};",(long)error.code, error.localizedDescription);
    ...
    if (location) {
        ...
        if (location.rgcData) {
            ...
            NSString *address = [NSString stringWithFormat:@"%@%@%@%@%@%@", kNotNull([location.rgcData country]), kNotNull([location.rgcData province]), kNotNull([location.rgcData city]), kNotNull([location.rgcData district]), kNotNull([location.rgcData town]), kNotNull([location.rgcData street])];
